...
- 目标：将结构、全文覆盖、报告逻辑、证据使用、实际渲染、独立复核和 Skill 文件分类门禁统一编排。
- 已完成：新增文件分类检查、独立复核检查；验证器复用真实媒体校验并忽略门禁产生的审计 JSON；总门禁加入材料追踪、主题决策、三种 renderer 和独立复核；构建完整 v3 单论文回放 fixture。
- 验证：8 项单元测试通过；完整 fixture 的 9 个脚本检查、Markdown/Obsidian/HTML 三种 renderer 和文件分类全部通过；材料 6/6、Evidence/Claim orphan 0、语义锚点 8、通读任务 1/1、独立复核 1/1；Skill 跟踪文件分类检查通过（119/119）。
